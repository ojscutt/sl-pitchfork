...
class emulator:
    def __init__(self, emulator_name):
        ...
        self.custom_objects = {"WMSE": WMSE(self.emulator_dict['custom_objects']['WMSE']['weights'])}
...
class ns():
    ...
    def logl(self, theta): 
        ...
        ll = -(len(m)*0.5*np.log(2*np.pi))-(0.5*self.log_sigma_det)-(0.5*np.einsum('ij, jk, ik->i', residual_matrix, self.sigma_inv, residual_matrix))
...
class ns_old():
    def __init__(self, priors, observed_vals, observed_unc, pitchfork, n_min=6, n_max=40, logl_scale = 1):
        self.priors = priors
        self.obs_val = observed_vals
