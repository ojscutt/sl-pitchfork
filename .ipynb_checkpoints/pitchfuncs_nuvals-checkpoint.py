...
class ns():
    def __init__(self, priors, observed_vals, observed_unc, pitchfork, logl_scale = 1, n_vals = [i+1 for i in range(5,40)]):
        self.priors = priors
        ...
        self.n_vals = [n_val-3 for n_val in n_vals]
    ...
    def logl(self, theta): 
        ...
        m = np.concatenate((m[0][:3], m[0][self.n_vals]))
